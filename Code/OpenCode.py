...
def send_servo_assigned(value): 
    ...
    arduino.write(b"@S%b\n" %valueb)
    servo_value = value
    print(value, lap_count, turn_count, turn_side)
#@log
...
LAB_BLACK_UPPER = np.array([85, 255, 255], dtype=np.uint8)
...
LEFT_EXIT_GROW_THRESH  = 2000     # the side that dropped must grow > 1200
RIGHT_EXIT_GROW_THRESH  = 2000
EXIT_TIME_SEC     = 0.5      # AND at least 10 seconds must pass since entry
EXIT_TIME_THRESH = 2.0
MAX_TIME_SEC 	  = 10.0
TURN_LEFT_ANGLE   = 41 # change lower if nessassary
TURN_RIGHT_ANGLE  = 81 # change higher if nessassary
MAX_TURN_RATE = 40
...
try:
    while True:
        ...
        if turn_count >= 12 and mode == MODE_WALL_FOLLOW:
            send_servo(66)
            sleep(2.0)
            break
        else:
            ...
            if mode == MODE_WALL_FOLLOW:
                ...
                if grew_ok and time_ok or time_max:
                    mode = MODE_WALL_FOLLOW
